...
namespace IslandGame.GameWorld
{
    class PathHandler
    {
        ...
        protected virtual List<BlockLoc> PathOLD(IslandPathingProfile startProfile, ref BlockLoc startLoc, HashSet<BlockLoc> goals, int heightOfEntity)
        {
            ...
            while (openNodes.size() > 0)
            {
                ...
                foreach (BlockLoc next in nextSteps)
                {
                   ...
                   /* else if (visitedLocations.Contains(toAdd.loc))
                    {
